...
export default function About({ data }: { data?: Record<string, unknown> }) {
  const d: AboutData = { ...defaults, ...(data as AboutData) }
  const bio = (d.bio as string[]) ?? defaults.bio!
  const skills = (d.skills as string[]) ?? defaults.skills!
      ...
      <div className="wrap">
        <div className="about-layout">
          ...
          <div className="fu d1">
            ...
            <h2 style={{ fontSize: 'clamp(1.9rem,3.8vw,3rem)', marginBottom: '22px' }}>{d.title}</h2>
            {bio.map((p, i) => (
              <p key={i} className="about-text">{p}</p>
            ))}
            <a href={d.linkedin_url} target="_blank" rel="noopener noreferrer" className="about-li-link">
